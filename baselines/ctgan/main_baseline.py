"""Project-compatible entrypoint for the CTGAN baseline."""
...
def load_training_data(config: dict[str, Any]) -> pd.DataFrame:
    ...
    if df.isna().any().any():
        raise ValueError(f"CTGAN input contains missing values: {train_path}")
    return df
...
def build_model(config: dict[str, Any], enable_gpu: bool):
    ...
    ctgan = config["ctgan"]
    model = CTGAN(
        embedding_dim=int(ctgan["embedding_dim"]),
        generator_dim=tuple(ctgan["generator_dim"]),
        discriminator_dim=tuple(ctgan["discriminator_dim"]),
        generator_lr=float(ctgan["generator_lr"]),
        generator_decay=float(ctgan["generator_decay"]),
        discriminator_lr=float(ctgan["discriminator_lr"]),
        discriminator_decay=float(ctgan["discriminator_decay"]),
        batch_size=int(ctgan["batch_size"]),
        discriminator_steps=int(ctgan["discriminator_steps"]),
        log_frequency=bool(ctgan["log_frequency"]),
        verbose=bool(ctgan["verbose"]),
        epochs=int(ctgan["epochs"]),
        pac=int(ctgan["pac"]),
        enable_gpu=enable_gpu,
    )
    model.set_random_state(int(ctgan["seed"]))
    return model
...
def train_and_sample(config: dict[str, Any]) -> tuple[Path, Path | None]:
    seed = int(config["ctgan"]["seed"])
    np.random.seed(seed)

    device = str(config["ctgan"]["device"])
    ...
    write_log(
        config,
        {
            ...
            "device": device,
            "epochs": int(config["ctgan"]["epochs"]),
            "timestamp": time.time(),
        },
    )

    discrete_columns = list(config["schema"]["discrete_columns"])
    started = time.time()
    ...
    sample_df = model.sample(int(config["ctgan"]["sample"]["num_samples"]))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", type=Path, default=Path("baselines/ctgan/config_baseline.yaml"))
...
def main() -> None:
    ...
    config = apply_dataset_overrides(config, "ctgan", args)
    if args.epochs is not None:
        config["ctgan"]["epochs"] = args.epochs
    if args.num_samples is not None:
        config["ctgan"]["sample"]["num_samples"] = args.num_samples
    if args.device:
        config["ctgan"]["device"] = args.device
    if args.seed is not None:
        config["ctgan"]["seed"] = args.seed

    result_path, loss_path = train_and_sample(config)
    print(f"wrote generated samples: {result_path}")
    if loss_path:
        print(f"wrote CTGAN loss values: {loss_path}")
